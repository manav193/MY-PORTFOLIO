...
function generateProjectFederationModule() {
  console.log('Generating local project federation module...');
  const manifests = JSON.parse(fs.readFileSync(path.join(FRONTEND_DIR, 'data', 'project-manifests.json'), 'utf8'));
  const knowledgeDocument = JSON.parse(fs.readFileSync(path.join(FRONTEND_DIR, 'data', 'project-knowledge.json'), 'utf8'));
  if (!Array.isArray(manifests) || !Array.isArray(knowledgeDocument.sources)) {
    throw new Error('Project federation data must contain a manifest array and knowledge sources array.');
  }
  const sourceByApplication = new Map(knowledgeDocument.sources.map(source => [source.application, source]));
  const definitions = manifests.map(manifest => {
    const knowledge = sourceByApplication.get(manifest.id);
    if (!knowledge) throw new Error(`Project manifest has no local knowledge source: ${manifest.id}`);
    return { manifest, knowledge };
  });
  if (sourceByApplication.size !== definitions.length) throw new Error('Project knowledge contains an unregistered module source.');
  const legacyProjects = definitions.flatMap(({ manifest, knowledge }) => knowledge.projects.map(project => ({
    id: project.id, name: project.name, aliases: project.aliases || [], category: project.category,
    type: project.type, tech: project.technologies || [], summary: project.summary,
    caseStudy: project.routes?.caseStudy,
    liveUrl: project.routes?.open || (manifest.entry.startsWith('http') ? manifest.entry : undefined)
  })));
  const output = `// Generated by build.js. Do not edit directly.\n`
    + `export const PROJECT_MANIFESTS = Object.freeze(${JSON.stringify(manifests, null, 2)});\n`
    + `export const PROJECT_KNOWLEDGE_SOURCES = Object.freeze(${JSON.stringify(knowledgeDocument.sources, null, 2)});\n`
    + `export const PROJECT_MODULE_DEFINITIONS = Object.freeze(${JSON.stringify(definitions, null, 2)});\n`
    + `export const LEGACY_PROJECTS_DB = Object.freeze(${JSON.stringify(legacyProjects, null, 2)});\n`;
  const targetDirectory = path.join(FRONTEND_DIR, 'js', 'generated');
  fs.mkdirSync(targetDirectory, { recursive: true });
  fs.writeFileSync(path.join(targetDirectory, 'project-federation.js'), output);
}
...
function buildCSS() {
  ...
  const cssFiles = ['styles.css', 'intro.css', 'arcade-os.css', 'project-page.css', 'boot-experience.css', 'adaptive-ui.css'];
...
async function main() {
  ...
  try {
    cleanDist();
    generateProjectFederationModule();
