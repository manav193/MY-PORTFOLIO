function addStyle(href){const style=document.createElement('link');style.rel='stylesheet';style.href=href;document.head.appendChild(style)}
...
export function initProjectLaunch(){
  ...
  const publicRemaining={
    'multi-api-system':'multiApi',
    'resume-ai':'resumeAi',
    'aurora-control-ui':'aurora',
    'shift-zero-ui':'shiftUi'
  };
  if(publicPage&&publicRemaining[publicId]){
    loadRemaining(publicRemaining[publicId],'../../assets/case-studies/remaining-launches.css');return;
  }
  ...
  const staticLaunches={
    ...
    'project-nimo.html':['nimo-launch.css','nimo-launch.js','nimo-launch-active'],
    'project-arcade-os.html':['arcade-launch.css','arcade-launch.js','arcade-launch-active']
  };
  const file=path.split('/').pop();
  if(staticLaunches[file]){
    const [css,js,activeClass]=staticLaunches[file];
    loadLaunch({css:`assets/case-studies/${css}`,js:`../../assets/case-studies/${js}`,activeClass});return;
  }
  const remainingStatic={
    'project-love-journey.html':'love',
    'project-nintendo.html':'nintendo',
    'project-nike.html':'nike',
    'project-velora-bites.html':'veloraUi'
  };
