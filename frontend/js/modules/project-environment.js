  ...
  if (!reduceMotion) {
    ...
    const updatePointer = (event) => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        body.style.setProperty('--px', `${x}%`);body.style.setProperty('--py', `${y}%`);body.style.setProperty('--sx', ((x - 50) / 10).toFixed(2));body.style.setProperty('--sy', ((y - 50) / 10).toFixed(2));
      });
    };
    addEventListener('pointermove', updatePointer, { passive: true });
    const sections = [...document.querySelectorAll('.cs-editorial-section')];
    const updateScroll = () => {const center = innerHeight / 2;for (const section of sections) {const rect = section.getBoundingClientRect();const distance = (rect.top + rect.height / 2 - center) / innerHeight;section.style.setProperty('--section-shift', `${Math.max(-8, Math.min(8, distance * -7))}px`);}};
    addEventListener('scroll', updateScroll, { passive: true });updateScroll();
  }
  document.querySelectorAll('.cs-overview-block,.cs-feature-grid li,.cs-tech-tag,.cs-split-half,.cs-hero-visual').forEach((card) => {card.dataset.envCard = 'true';card.addEventListener('pointermove', (event) => {const rect = card.getBoundingClientRect();card.style.setProperty('--card-x', `${event.clientX - rect.left}px`);card.style.setProperty('--card-y', `${event.clientY - rect.top}px`);}, { passive: true });});
}
